...
import { Designer, Collection } from "../../components/types";
...
const DesignerPage: React.FC = () => {
  ...
  const { designers, collections, loading, error, dispatch } = useContext(ApiContext) || {};
  ...
  const designerCollections = collections!.filter((collection: Collection) => collection.designerId === designerId);
  ...
  const deleteHandler = async () => {
    try {
      ...
      if (dispatch) {
        dispatch({ type: 'REMOVE_DESIGNER', payload: designer.id });
      }
